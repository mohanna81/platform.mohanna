...
export default function ConsortiaPage() {
  ...
  // Only show "Manage Consortia" button for Super_user and Admin
  // Facilitator now has same view as Organization User
  const canManageConsortia = user && (user.role === 'Super_user' || user.role === 'Admin');

  return (
    <Layout>
      <div className="px-4 py-8 sm:px-8 md:px-12 md:py-12">
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-8">
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-[#0b1320]">Consortia</h1>
          {canManageConsortia && (
            <Button
              variant="primary"
              size="md"
